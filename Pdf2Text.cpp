// This code converts a pdf file to text
// I downloaded from: http://www.codeproject.com/KB/cpp/ExtractPDFText.aspx
...
float ExtractNumber(const char* search, int lastcharoffset)
{
	...
	float flt=-1.0;
	char buffer[oldchar+5]; memset(buffer, 0, sizeof(buffer));
...
void Pdf2Text(const char* pdf, const char* text)
{
	...
	FILE* fileo = fopen(text, "w");
	if (fileo) fclose(fileo);
	fileo = fopen(text, "a");

	//Open the PDF source file:
	FILE* filei = fopen(pdf, "rb");
	...
	if (filei && fileo)
	{
		...
		char* buffer = new char [filelen]; memset(buffer, 0, filelen);
